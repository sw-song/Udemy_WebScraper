...
class UdmScraper():
    ...
    def scrap_links(self):
        ...
        for page in range(1, pages+1):
            ...
            course_link_elements = driver.find_elements_by_css_selector(
                '.course-list--container--3zXPS > .popper--popper--19faV > a')
        ...
        course_links.to_csv('./data/links_{}.csv'.format(keyword))
    ...
    def make_Df(self):
        ...
        course_links = pd.read_csv('./data/links_{}.csv'.format(keyword))
        ...
        for course_link in course_links['link']:
            ...
            try:
                ...
                udemy_df.to_csv('./data/udemy_df_{}.csv'.format(keyword), index=False, mode='w')
                print('파일을 수정했습니다.')
    ...
    def preprocessing(self):
        keyword = self.keyword
        udemy_df = pd.read_csv('./data/udemy_df_{}.csv'.format(keyword))
